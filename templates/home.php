...
$total_redirects = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM redirect_rules WHERE disabled != %d", 1));
$total_sandbox_websites = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM websites WHERE is_prod = %d AND disabled != %d", 0, 1));
$total_prod_websites = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM websites WHERE is_prod = %d AND disabled != %d", 1, 1));
$total_servers = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM servers WHERE disabled != %d", 1));
$last_modified_rule = $wpdb->get_row($wpdb->prepare("SELECT * FROM redirect_rules ORDER BY last_modified_date DESC LIMIT %d", 1), ARRAY_A);
$last_created_rule = $wpdb->get_row($wpdb->prepare("SELECT * FROM redirect_rules ORDER BY created_date DESC LIMIT %d", 1), ARRAY_A);
        ...
        <div class="dashboard-grid">
            ...
            <div class="dashboard-grid-card last-created-card">
                ...
                <span class="card-description redirect-rule-path">
                    <?php echo get_full_from_url($last_created_rule) . '<i class="fa-solid fa-arrow-right-long"></i>' . $last_created_rule['to_url']; ?>
                </span>
            </div>
            <div class="dashboard-grid-card last-modified-card">
                <h3 class="card-title">Last Modified</h3>
                <span class="card-description redirect-rule-path">
                    <?php echo get_full_from_url($last_modified_rule) . '<i class="fa-solid fa-arrow-right-long"></i>' . $last_modified_rule['to_url']; ?>
                </span>
            </div>
        </div>
    </div>
