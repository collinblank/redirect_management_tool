    ...
    <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="POST" class="modal-form">
        ...
        <ul class="form-items-list">
            <li class="form-item">
                ...
                <input type="text" class="form-text-input" id="redirect-rule-name" name="redirect_rule_name" placeholder="ex. Pastor Appreciation" value="<?php echo $redirect_rule_to_edit['name'] ?? "" ?>" tabindex="1" minlength="4" maxlength="50" pattern="^[A-Za-z]+(?: [A-Za-z]+)*$">
                <p class="form-validation-msg"></p>
            </li>
            <li class="form-item">
                <label for="redirect-rule-description" class="form-label">Description</label>
                <textarea id="redirect-rule-description" class="form-textarea" name="redirect_rule_description" placeholder="ex. Redirect for churches and pastor appreciation week" tabindex="2"><?php echo $redirect_rule_to_edit['description'] ?? "" ?></textarea>
                <p class="form-validation-msg"></p>
            </li>
            <li class="form-item">
